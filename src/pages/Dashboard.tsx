...
const metricsConfig = {
  ...
  revenue: { label: "Revenus", color: "#a855f7", icon: TrendingUp, type: "area" },
...
const activities = [
  { id: 1, type: "ride", label: "Trajet publié", detail: "Tana → Majunga", date: "Il y a 2h" },
  { id: 2, type: "booking", label: "Réservation confirmée", detail: "2 passagers", date: "Il y a 3h" },
  { id: 3, type: "message", label: "Nouveau message", detail: "Discussion avec Jean", date: "Il y a 5h" },
]
...
function getActivityIcon(type: string) {
  switch (type) {
    case "ride": return <Car className="w-4 h-4 text-emerald-500" />
...
type SectionKey = "chart" | "bookings" | "reviews" | "activity"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-sm mt-2 text-muted-foreground">Trajets publiés</p>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-sm mt-2 text-muted-foreground">Réservations</p>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-sm mt-2 text-muted-foreground">Note moyenne</p>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-sm mt-2 text-muted-foreground">Revenus</p>
        ...
        <div className="h-[260px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              {visible.rides && (
                <Line type="monotone" dataKey="rides" stroke={metricsConfig.rides.color} strokeWidth={2} dot={{ r: 3 }} />
              )}
              {visible.bookings && (
                <Line type="monotone" dataKey="bookings" stroke={metricsConfig.bookings.color} strokeWidth={2} dot={{ r: 3 }} />
              )}
              {visible.occupancy && (
                <Line type="monotone" dataKey="occupancy" stroke={metricsConfig.occupancy.color} strokeWidth={2} />
              )}
              {visible.conversion && (
                <Line type="monotone" dataKey="conversion" stroke={metricsConfig.conversion.color} strokeWidth={2} />
              )}
              {visible.messages && (
                <Line type="monotone" dataKey="messages" stroke={metricsConfig.messages.color} strokeWidth={2} />
              )}
              {visible.reviews && (
                <Line type="monotone" dataKey="reviews" stroke={metricsConfig.reviews.color} strokeWidth={2} />
              )}
            </AreaChart>
          </ResponsiveContainer>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {activities.slice(0, 10).map((activity) => (
                <tr key={activity.id} className="border-t hover:bg-muted/40 transition">
                  ...
                  <td className="px-3 py-2 text-muted-foreground text-xs">{activity.detail}</td>
                  <td className="px-3 py-2 text-right text-xs text-muted-foreground">{activity.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
